...
| **`Shop/`** | **ShopService.luau** — shop remotes (catalog, buy weapon/ammo), trader prompt. **AimResolver.luau** — aim state, fire-cone validation, shot ray/direction, weapon raycast params. **EnemyDamageService.luau** — apply damage + feedback + DOT (money/kill reward, blink, damage indicators, crits, burn). **SentryTurretController.luau** — turret placement, deploy/repair, combat loop, healthbar UI, targeting. **CatalogAndPricing.luau** — build catalog for player. **InventoryIndex.luau** — index owned tools/sentries. **PurchaseFlow.luau** — purchase validation. **WeaponAmmoRuntime.luau** — weapon ammo state. **SentryRuntime.luau** — sentry state. **WeaponToolSetup.luau** — tool creation from templates, dual-wield visuals/pose, template lookup. **WeaponVfx.luau** — weapon effects folder and VFX (tracer, muzzle flash, flame puff, explosion). **ShopConstants.luau** — shared constants (fire cone, sentry limits, turret defaults). |
